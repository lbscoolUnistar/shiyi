<link rel="stylesheet" href="${ctx}/static/amazeUI/css/amazeui.min.css"/>
  <link rel="stylesheet" href="${ctx}/static/amazeUI/css/admin.css">
  <!--[if lt IE 9]>
  <script src="http://libs.baidu.com/jquery/1.11.1/jquery.min.js"></script>
  <script src="http://cdn.staticfile.org/modernizr/2.8.3/modernizr.js"></script>
  <script src="${ctx}/static/amazeUI/js/amazeui.ie8polyfill.min.js"></script>
  <![endif]-->

  <!--[if (gte IE 9)|!(IE)]><!-->
  <script src="${ctx}/static/jQuery/jquery-3.1.1.min.js"></script>
  <!--<![endif]-->
  <script src="${ctx}/static/amazeUI/js/amazeui.min.js"></script>
  <script src="${ctx}/static/amazeUI/js/app.js"></script>